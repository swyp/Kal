...
- (void)drawRect:(CGRect)rect
{
  ...
  CGContextTranslateCTM(ctx, 0, [KalGridView tileSize].height);
  CGContextScaleCTM(ctx, 1, -1);
  
  if ([self isToday] && self.selected) {
    [[[UIImage imageNamed:@"Kal.bundle/kal_tile_today_selected.png"] stretchableImageWithLeftCapWidth:6 topCapHeight:0] drawInRect:CGRectMake(0, -1, [KalGridView tileSize].width+1, [KalGridView tileSize].height+1)];
    textColor = [UIColor whiteColor];
    shadowColor = [UIColor blackColor];
    markerImage = [UIImage imageNamed:@"Kal.bundle/kal_marker_today.png"];
  } else if ([self isToday] && !self.selected) {
    [[[UIImage imageNamed:@"Kal.bundle/kal_tile_today.png"] stretchableImageWithLeftCapWidth:6 topCapHeight:0] drawInRect:CGRectMake(0, -1, [KalGridView tileSize].width+1, [KalGridView tileSize].height+1)];
    textColor = [UIColor whiteColor];
    shadowColor = [UIColor blackColor];
    markerImage = [UIImage imageNamed:@"Kal.bundle/kal_marker_today.png"];
  } else if (self.selected) {
    [[[UIImage imageNamed:@"Kal.bundle/kal_tile_selected.png"] stretchableImageWithLeftCapWidth:1 topCapHeight:0] drawInRect:CGRectMake(0, -1, [KalGridView tileSize].width+1, [KalGridView tileSize].height+1)];
    textColor = [UIColor whiteColor];
    shadowColor = [UIColor blackColor];
  ...
  textY = 6.f + roundf(0.5f * ([KalGridView tileSize].height - textSize.height));
  if (shadowColor) {
    [shadowColor setFill];
  ...
  if (self.highlighted) {
    ...
    CGContextFillRect(ctx, CGRectMake(0.f, 0.f, [KalGridView tileSize].width, [KalGridView tileSize].height));
  }
}
